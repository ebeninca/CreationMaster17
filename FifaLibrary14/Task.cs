...
namespace FifaLibrary
{
    public class Task
    {
        ...
        public void LinkTrophy(Trophy ownerTrophy)
        {
            ...
            this.m_Team = (Team)null;
            Compobj compobj = (Compobj)FifaEnvironment.CompetitionObjects.SearchId(this.m_Parameter1);
            //failsafe for new tournaments, avoid restart app to add rankings table
            if (compobj == null)
            {
                FifaEnvironment.CompetitionObjects.SortId();
                compobj = (Compobj)FifaEnvironment.CompetitionObjects.SearchId(this.m_Parameter1);
            }
            if (compobj == null || !compobj.IsGroup())
                return;
            this.m_Group = (Group)compobj;
        }
